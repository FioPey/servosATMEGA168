...
#define CLOCKS_PER_25 5000
#define ANGLE1 25//115.0// recta a 25º
#define ANGLE2 90.0// recta a 25º
...
ISR (TIMER1_COMPA_vect)//Cada vegada que el comptador arribi al que se li ha dit es crida aquesta funciò
{
	if(n_cicles==0)
	{
		if(puja)//En HIGH
		{
			OCR1A=CLOCKS_PER_25-N_CLOCKS1;//interrupcciò al temps que falta
	...
	else if(n_cicles==1)
	{
		if(puja)//En HIGH
		{
			OCR1A=CLOCKS_PER_25-N_CLOCKS2;//interrupcciò al temps que falta
			PORTB=0x08;					//pin 4 que és el bit 5 el posa en high
			puja=FALSE;
		}
		else
		{
			OCR1A=CLOCKS_PER_25;//Torna a posar al cicle el que falta
			PORTB=0;		//Apaga el pols
			puja=TRUE;
			n_cicles++;
		}
